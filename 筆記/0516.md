...
```
* 腳本`test.py`(home/user/pox/pox/test.py)
```py
...
mininet> h1 ping h2
```
### 範例三 DNAT (port forwarding)
![範例](../IMAGE/ubundu0516_3.png)

* `h2`連線至`1.2.3.5:8888`，switch 會把封包轉址到`h1`，存取網頁內容 
```
# python 2.py
mininet> xterm h1 h2
...
```
### 範例四 Firewall
#### 使`h1`可與`h2`互 ping，也可連到`h2:80`，但`h2:8080`則不行
```
# python 2.py
mininet> xterm h1
