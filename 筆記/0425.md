...
```
## SSH Tunnel 的應用
### 範例一 透過 SSH Tunnel 連到外部網路
![範例](../IMAGE/ubundu0425_2.png)
#### 腳本`lab2.py`
```py
#!/usr/bin/python
...
```
### 範例二 從外部網路透過 SSH Tunnel 存取內部
* Remote Port Forwarding：從裡面先連到外面，由外部反向建立一個 SSH Tunnel
![範例](../IMAGE/ubundu0425_3.png)
#### 腳本`lab3.py`
```py
#!/usr/bin/python
from mininet.net import Containernet
